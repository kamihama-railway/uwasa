...
func BenchmarkOptimizationComparison(b *testing.B) {
	...
	for _, sc := range scenarios {
		...
		b.Run(sc.name+"/NeoEx", func(b *testing.B) {
			engine, err := NewEngineVMNeoEx(sc.input)
			if err != nil {
				b.Fatalf("NewEngineVMNeoEx error: %v", err)
			}
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				_, _ = engine.Execute(sc.vars)
...
func BenchmarkMixedTypeArithmetic(b *testing.B) {
	...
	b.Run("IntFloat_NeoEx", func(b *testing.B) {
...
func BenchmarkConcatBuiltin(b *testing.B) {
	...
	b.Run("Variables_NeoEx", func(b *testing.B) {
		engine, _ := NewEngineVMNeoEx(input)
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, _ = engine.Execute(vars)
...
func BenchmarkStringConcatenation(b *testing.B) {
	...
	b.Run("ConstStrings_NeoEx", func(b *testing.B) {
		engine, _ := NewEngineVMNeoEx(input)
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, _ = engine.Execute(vars)

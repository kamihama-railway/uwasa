...
## 寄存器虚拟机 (Experimental RVM)

除了标准的栈式 VM，Uwasa 还提供了一个实验性的寄存器式虚拟机 (Register-based VM)。
- **更少的移动**: 在某些复杂表达式中，寄存器架构可以减少中间值的推拉次数。
- **独立包**: 位于 `rvm/` 目录下，保持极简和独立性。

通过调用 `engine.UseRegisterVM()` 可以启用该引擎。

## 适用场景

- **高吞吐量系统**: 默认 `NewEngine` 已启用栈式 VM。
- **变量密集型规则**: 可以尝试启用 `UseRegisterVM()`。
- **极简或快速变化规则**: AST 引擎具有更短的初始化耗时，适合只执行一次的规则。

...
## 3. 虚拟机编译器 (VMCompiler) 中的集成

值得注意的是，`NewEngineVM` 使用的 `VMCompiler` 也集成了类似的优化逻辑。它会在字节码生成前执行以下流水线：
1. **Fold**: 基础常量折叠。
2. **Simplify**: 基于代数规律的 AST 精简（同 Recompiler 逻辑）。
3. **Peephole**: 针对字节码的专用指令融合优化。

因此，当你使用 `NewEngineVM` 时，即便不显式开启 `UseRecompiler` 选项，也能享受到大多数核心代数简化的红利。

...
- **Level 3: 指令融合 (VM Fusion)**: 仅在 VM 引擎中启用，将生成的字节码指令进行模式匹配并合并。

### 3.6 虚拟机 (VM) 开发指南
VM 的核心位于 `vm.go` 和 `vm_compiler.go`。
- **指令扩展**: 若需增加新指令，需在 `bytecode.go` 定义 `OpCode`，在 `VMCompiler.walk` 中生成该指令，并在 `RunVM` 的调度循环中实现逻辑。
- **添加融合规则**: 在 `VMCompiler.peephole` 函数中通过检查 `c.instructions` 的滑动窗口添加新的融合逻辑。
- **性能红线**: 确保 VM 调度循环内的逻辑尽可能精简，避免任何非必要的内存分配或分支预测失败。

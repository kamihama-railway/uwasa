...
- **动作模式 (`if <cond> then <action>`)**:
    - **返回值**: `any` 或 `nil`
    - **来源**: 若 `<cond>` 为真，执行并返回 `<action>` 的计算结果；若为假，**不执行**动作并直接返回 `nil`。由于支持副作用，这通常用于有条件的变量修改。

### 2.2 赋值表达式 (Assignment)
- **语法**: `name = expression`
- **返回值**: `any`
- **来源**: 返回右侧 `expression` 的求值结果。这使得链式赋值（如 `a = b = 1`）在引擎中是合法且确定的。
...
- **字符串拼接**:
    - **条件**: 使用 `+` 且两边均为 `string`。
    - **返回值**: `string`
    - **来源**: 两个字符串连接后的新字符串。注意：如果其中一个是数字，引擎在 `OptBasic` 下会尝试拼接，但在 `UseRecompiler` 模式下会进行静态类型检查并建议使用 `concat`。
...
Uwasa 提供分层优化机制，通过 `EngineOptions` 进行配置。

### 3.1 基础优化等级 (OptimizationLevel)

- **OptNone (不优化)**: 直接对解析后的原始 AST 进行遍历求值。适用于调试或单次执行。
- **OptBasic (基础优化 - 默认)**:
    - **常量折叠 (Constant Folding)**: 在编译阶段计算字面量子树。
    - **布尔短路折叠**: 对涉及变量的逻辑运算进行预先折叠（如 `false && expr`）。

### 3.2 独立再编译器 (Recompiler)

`Recompiler` 是一个独立于基础优化等级的可选组件（通过 `UseRecompiler: true` 开启），它提供深度的代数简化和严格的静态分析。更多详细行为和变量关系处理，请参阅 [Recompiler 深度解析](./recompiler.md)。
...
- `x * 1` -> `x`
- `x * 0` -> `0`
- `x - x` -> `0` (自动检测并跳过含副作用的表达式)
- `x == x` -> `true`
- `a = a` -> `a` (自赋值消除)

#### 增强的静态检查 (Static Analysis)
在编译阶段识别潜在的运行时错误或逻辑风险：
- **除零检测**: 检测显式的常数除法错误。
- **严格类型检查**:
    - 检测字符串与数值的不合法混合运算。
    - 检测布尔类型与非布尔类型的逻辑运算（如 `1 && true`）。
    - 检测不合法的布尔算术运算（如 `true + 1`）。
- **不可达代码检测**: 识别 `if (常量)` 结构导致的永远不会执行的分支。
...
opts := uwasa.EngineOptions{
    OptimizationLevel: uwasa.OptBasic,
    UseRecompiler:     true, // 开启独立再编译
}
engine, err := uwasa.NewEngineWithOptions(`(a + 0) * 1 + (b - b)`, opts)
// 经过 Recompiler 后，AST 实际上简化为了标识符 `a` 的求值。
```
...
在开启 `UseRecompiler` 时，引擎会返回更详细的编译时错误：

...
4. **执行引擎**:
    - **AST 解释器 (Evaluator)**: 递归遍历优化后的 AST 并根据上下文计算结果，适用于单次执行或轻量规则。
    - **原生虚拟机 (VM)**: 将 AST 编译为字节码并由高性能核心执行，适用于高吞吐量和延迟敏感场景。
...
为了追求极致性能，Uwasa 引入了基于字节码的虚拟机。

### 1. 数据路径分离
VM 路径与传统的 AST 路径基本完全分离。`NewEngineVM` 拥有其专属的 `VMCompiler`，能够进行针对字节码特性的深度优化。

### 2. 指令融合 (Peephole Optimization)
VM 编译器通过滑动窗口扫描生成的原始指令，将多个基础指令（推栈、弹出、基本运算）合并为单一的融合指令（如 `OpEqualGlobalConst`），减少了循环分发的开销。

### 3. 类型安全与零装箱
VM 内部使用 `Value` 联合结构体处理数据，完全避免了在算术运算和比较过程中对 `interface{}` 的频繁装箱和拆箱。

...
func RunNeoVMWithMap(bc *NeoBytecode, vars map[string]any) (any, error) {
	...
	for pc < nInsts {
		...
		case NeoOpGetGlobal:
			...
			switch v := val.(type) {
			case int64:
				*target = Value{Type: ValInt, Num: uint64(v)}
			case int:
				*target = Value{Type: ValInt, Num: uint64(int64(v))}
			case float64:
				*target = Value{Type: ValFloat, Num: math.Float64bits(v)}
			case string:
				*target = Value{Type: ValString, Str: v}
			case bool:
				*target = Value{Type: ValBool, Num: boolToUint64(v)}
			case nil:
				*target = Value{Type: ValNil}
			default:
				*target = FromInterface(v)
			}
		case NeoOpSetGlobal:
			name := (*Value)(unsafe.Add(unsafe.Pointer(pConsts), uintptr(inst.Arg)*valSize)).Str
		...
		case NeoOpEqualGlobalConst:
			...
			switch v := val.(type) {
			case int64:
				res = cv.Type == ValInt && v == int64(cv.Num)
			case float64:
				res = cv.Type == ValFloat && v == math.Float64frombits(cv.Num)
			case string:
				res = cv.Type == ValString && v == cv.Str
			default:
				res = EqualAny(val, cv.ToInterface())
			}
			sp++
			if sp >= 64 {
				return nil, fmt.Errorf("NeoVM stack overflow")
		...
		case NeoOpAddGlobal, NeoOpAddGC:
			...
			case int64:
				if cv.Type == ValInt {
					*target = Value{Type: ValInt, Num: uint64(v + int64(cv.Num))}
				} else if cv.Type == ValFloat {
					*target = Value{Type: ValFloat, Num: math.Float64bits(float64(v) + math.Float64frombits(cv.Num))}
				} else {
					*target = AddAny(v, cv.ToInterface())
				}
			case float64:
				if cv.Type == ValInt {
					*target = Value{Type: ValFloat, Num: math.Float64bits(v + float64(int64(cv.Num)))}
				} else if cv.Type == ValFloat {
					*target = Value{Type: ValFloat, Num: math.Float64bits(v + math.Float64frombits(cv.Num))}
				} else {
					*target = AddAny(v, cv.ToInterface())
				}
			case string:
				if cv.Type == ValString {
					*target = Value{Type: ValString, Str: v + cv.Str}
				} else {
					*target = AddAny(v, cv.ToInterface())
				}
			default:
		...
		case NeoOpGreaterGlobalConst:
			...
			case int64:
				if cv.Type == ValInt {
					...
					res = float64(v) > math.Float64frombits(cv.Num)
				} else {
					res = GreaterAny(v, cv.ToInterface())
				}
			case float64:
				if cv.Type == ValInt {
					res = v > float64(int64(cv.Num))
				} else if cv.Type == ValFloat {
					res = v > math.Float64frombits(cv.Num)
				} else {
					res = GreaterAny(v, cv.ToInterface())
				}
			default:
				res = GreaterAny(val, cv.ToInterface())
			}
			stack[sp] = Value{Type: ValBool, Num: boolToUint64(res)}
		case NeoOpLessGlobalConst:
			...
			default:
				res = LessAny(val, cv.ToInterface())
			}
			stack[sp] = Value{Type: ValBool, Num: boolToUint64(res)}
		case NeoOpAddGlobalGlobal:
			...
			if i1, ok1 := v1.(int64); ok1 {
				if i2, ok2 := v2.(int64); ok2 {
					stack[sp] = Value{Type: ValInt, Num: uint64(i1 + i2)}
					continue
				}
			}
		...
		case NeoOpAddC:
			l := &stack[sp]
			cv := (*Value)(unsafe.Add(unsafe.Pointer(pConsts), uintptr(inst.Arg)*valSize))
			if l.Type == ValInt && cv.Type == ValInt {
				l.Num += cv.Num
			} else {
				*l = l.Add(*cv)
			}
		...
		case NeoOpConcat2:
			...
			if l.Type == ValString {
				...
				s1 = fmt.Sprintf("%v", l.ToInterface())
			}
			if r.Type == ValString {
				s2 = r.Str
			} else {
				s2 = fmt.Sprintf("%v", r.ToInterface())
			}
		...
		case NeoOpConcatGC:
			...
			if cv.Type == ValString {
				...
				s2 = fmt.Sprintf("%v", cv.ToInterface())
			}
		...
		case NeoOpConcatCG:
			...
			if cv.Type == ValString {
				...
				s1 = fmt.Sprintf("%v", cv.ToInterface())
			}
...
func runNeoVMGeneral(bc *NeoBytecode, ctx Context) (any, error) {
	...
	for pc < nInsts {
		...
		case NeoOpFusedCompareGlobalConstJumpIfFalse:
			...
			cv := (*Value)(unsafe.Add(unsafe.Pointer(pConsts), uintptr(cIdx)*valSize))
			val, _ := ctx.Get(name)
			if !EqualAny(val, cv.ToInterface()) {
				pc = jTarget
			}
		case NeoOpFusedGreaterGlobalConstJumpIfFalse:
			gIdx := int(inst.Arg>>22) & 0x3FF
			...
			cv := (*Value)(unsafe.Add(unsafe.Pointer(pConsts), uintptr(cIdx)*valSize))
			val, _ := ctx.Get(name)
			if !GreaterAny(val, cv.ToInterface()) {
				pc = jTarget
			}
		case NeoOpFusedLessGlobalConstJumpIfFalse:
			gIdx := int(inst.Arg>>22) & 0x3FF
...
func (l Value) Greater(r Value) bool {
	if l.Type == ValInt && r.Type == ValInt {
		return int64(l.Num) > int64(r.Num)
	}
	lf, okL := valToFloat64(l)
	rf, okR := valToFloat64(r)
	if okL && okR {
...
func (l Value) Add(r Value) Value {
	if l.Type == ValInt && r.Type == ValInt {
		return Value{Type: ValInt, Num: l.Num + r.Num}
	}
	if l.Type == ValString && r.Type == ValString {
		return Value{Type: ValString, Str: l.Str + r.Str}
	}
	lf, _ := valToFloat64(l)
	rf, _ := valToFloat64(r)
...
func (l Value) Div(r Value) Value {
	if (r.Type == ValInt && r.Num == 0) || (r.Type == ValFloat && math.Float64frombits(r.Num) == 0) {
		return Value{Type: ValFloat, Num: math.Float64bits(math.Inf(1))}
	}
	if l.Type == ValInt && r.Type == ValInt {
		return Value{Type: ValInt, Num: l.Num / r.Num}
	}
...
func (l Value) ModErr(r Value) (Value, error) {
	if r.Type != ValInt {
		return Value{}, fmt.Errorf("modulo operator supports only integers")
	}
	if r.Num == 0 {
		return Value{}, fmt.Errorf("division by zero")
	}
...
func AddAny(v1, v2 any) Value {
	...
	case string:
		if rv, ok := v2.(string); ok {
			return Value{Type: ValString, Str: lv + rv}
		}
	}
